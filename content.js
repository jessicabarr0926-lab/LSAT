...
window.JESSI_PREPS_DATA = {
  ...
  navigation: [
    ...
    { route: "roadmap", label: "Gap Analysis", eyebrow: "7Sage + Demon Benchmark" },
  ],
  competitiveGaps: {
    summary:
      "The high-leverage difference is not content volume by itself. 7Sage and LSAT Demon reduce decision fatigue, force review discipline, and keep students inside a measurable loop: diagnose, drill, blind review, repeat.",
    buildFirst: [
      {
        rank: 1,
        title: "Timed Section mode",
        cause: "Without a 35-minute proctored section, practice stays fragmented.",
        effect: "Students cannot build pacing stamina or compare section-level performance over time.",
        action: "Build LR and RC section runners with timer, pause rules, resume state, and section review.",
      },
      {
        rank: 2,
        title: "Adaptive Drill button + engine",
        cause: "Manual drill choice makes the student decide what the app should decide.",
        effect: "Weak areas get avoided and study sessions start slowly.",
        action: "Use attempts, misses, confidence, and recency to auto-build the next drill.",
      },
      {
        rank: 3,
        title: "Forced Blind Review workflow",
        cause: "Showing explanations too early lets students consume answers instead of diagnosing thinking.",
        effect: "Blind-review gap remains a vanity metric instead of a habit-changing loop.",
        action: "Require re-answer, confidence, reasoning note, and trap tag before explanation unlocks.",
      },
      {
        rank: 4,
        title: "Per-family mastery rating",
        cause: "One readiness score hides the difference between LR, RC, and question families.",
        effect: "Students cannot see whether Flaw, Assumption, RC Structure, or pacing is driving the score.",
        action: "Create 0-100 family ratings weighted by difficulty, accuracy, timing, and recent trend.",
      },
      {
        rank: 5,
        title: "Score-trajectory chart",
        cause: "A snapshot cannot show whether the system is working.",
        effect: "Motivation drops because progress feels invisible between full tests.",
        action: "Track timed section, drill, and full-test results over time with range and variance.",
      },
      {
        rank: 6,
        title: "Persistent left-rail navigation",
        cause: "Hidden navigation increases friction and makes the product feel less like a study cockpit.",
        effect: "Core loops are harder to discover and resume.",
        action: "Keep the current sidebar, then add route groups and in-progress badges.",
      },
      {
        rank: 7,
        title: "Streak counter + daily return mechanics",
        cause: "There is no immediate reward for showing up.",
        effect: "Study consistency depends on willpower instead of product design.",
        action: "Add streak, week strip, due-today count, and recovery rules.",
      },
      {
        rank: 8,
        title: "Study plan with test date",
        cause: "The app does not know the user's deadline.",
        effect: "Recommendations cannot balance urgency, weak areas, and remaining weeks.",
        action: "Capture test date, target score, weekly hours, and generate weekly blocks.",
      },
      {
        rank: 9,
        title: "Wrong-answer journal editor",
        cause: "A journal KPI exists, but the editor is not yet strong enough to shape behavior.",
        effect: "Misses become records instead of reusable rules.",
        action: "Add rich notes, trap tags, question links, review date, and reusable templates.",
      },
      {
        rank: 10,
        title: "Practice Test mode",
        cause: "Students need full-test endurance and score conversion practice.",
        effect: "Section skills may not transfer to full exam conditions.",
        action: "Build a 4-section simulator using original local questions plus official-material links.",
      },
      {
        rank: 11,
        title: "Section and drill history",
        cause: "Completed work is not presented as a searchable learning record.",
        effect: "Students cannot resume, compare, archive, note, or bookmark prior sessions.",
        action: "Create history rows with score, time, notes, BR status, bookmarks, and filters.",
      },
      {
        rank: 12,
        title: "Explanations library",
        cause: "Explanations exist in fragments instead of a top-level searchable index.",
        effect: "Review becomes harder when students need to find a pattern across questions.",
        action: "Add search by family, trap, difficulty, section, and linked lesson.",
      },
    ],
    categories: [
      {
        name: "Core study mechanics",
        currentGap: "JessiPreps has lessons, drills, and tests, but needs stronger one-click adaptive study, timed section rigor, full PT simulation, resume state, bookmarks, and embedded lesson practice.",
        causeEffect: "Cause: too many user choices at session start. Effect: weaker completion, less pacing data, and less trust in recommendations.",
        nextMove: "Prioritize adaptive drill, timed sections, practice blocks, resume state, and bookmarks.",
      },
      {
        name: "Spaced repetition and review",
        currentGap: "The dashboard now has a due-today review card, but Blind Review still needs a required workflow before explanations unlock.",
        causeEffect: "Cause: review is optional and mostly record-based. Effect: students see misses but may not change the habit that produced them.",
        nextMove: "Build forced Blind Review, due-today SRS, section review tools, variance, and explanation search.",
      },
      {
        name: "Analytics and data depth",
        currentGap: "Top-line KPIs exist, but 7Sage and Demon go deeper with priorities, question-type analytics, trend charts, time buckets, and study quality.",
        causeEffect: "Cause: data is not yet sliced by behavior and family. Effect: students know their score but not the exact cause of score movement.",
        nextMove: "Add family mastery, score trend, time-on-question, variance, official-test count, and study-quality score.",
      },
      {
        name: "Community and social",
        currentGap: "There is no forum, live class system, office hours, recorded sessions archive, profiles, chats, or leaderboard.",
        causeEffect: "Cause: community features require people and moderation, not just UI. Effect: building them too early would dilute core product work.",
        nextMove: "Keep below the line until the core study loop proves retention.",
      },
      {
        name: "Study planning",
        currentGap: "A plan page exists, but it needs test-date awareness, end-date planning, skip confirmations, and weekly workload logic.",
        causeEffect: "Cause: the app does not yet know the deadline. Effect: daily recommendations cannot adapt to urgency.",
        nextMove: "Capture LSAT date, target score, available hours, and generate weekly blocks.",
      },
      {
        name: "Admissions workflow",
        currentGap: "No admissions module, school tracker, decision tracker, scholarship estimator, or post-LSAT workflow.",
        causeEffect: "Cause: admissions scope is broad and downstream from LSAT readiness. Effect: it is valuable later but distracting now.",
        nextMove: "Defer until the learning product has traction; start later with a lightweight school list and score-goal planner.",
      },
      {
        name: "UX and on-screen tooling",
        currentGap: "The SPA has a persistent sidebar and accessibility toggles, but needs streaks, date filters, notifications, KPI tooltips, onboarding, richer notes, and privacy controls.",
        causeEffect: "Cause: important cues are missing or passive. Effect: students do not get enough guidance when they are new, stuck, or returning.",
        nextMove: "Add streak, onboarding, tooltip explainers, empty-state coaching, and cookie/privacy controls.",
      },
      {
        name: "Content depth",
        currentGap: "The site now has 127 MP4 lessons and original questions, but it cannot legally mirror the full official PrepTest pool without licensing.",
        causeEffect: "Cause: official LSAT content is licensed. Effect: JessiPreps should compete on original explanations, strategy, review quality, and official-link workflows.",
        nextMove: "Frame the bank as original skill-building and add import-your-own scores/data.",
      },
      {
        name: "Pricing and tier surfaces",
        currentGap: "No subscribe CTA, premium gating, free-account gates, or tier messaging.",
        causeEffect: "Cause: monetization surfaces are not designed yet. Effect: users do not understand what is free, premium, or worth upgrading for.",
        nextMove: "Add soft upgrade prompts only after core feature value is visible.",
      },
      {
        name: "Polish and trust touches",
        currentGap: "The product needs stronger hover states, feedback widgets, official-material tracking, refined typography, and less lavender dominance.",
        causeEffect: "Cause: visual hierarchy still reads more soft-study than serious cockpit. Effect: the product can feel less rigorous than 7Sage or Demon.",
        nextMove: "Tighten palette, add feedback controls, and make drill rows show time and completion state.",
      },
    ],
  },
  settings: [
