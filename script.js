...
function requestedLessonDescription(title, type, skill) {
  const lowerType = type.toLowerCase();
  if (lowerType.includes("worked")) {
    return `A website-ready worked example for ${title}: identify the task, predict the answer job, eliminate traps, and finish with a linked ${skill.toLowerCase()} drill.`;
  }
  if (lowerType.includes("quick")) {
    return `A concise guide for ${title}: the minimum method, the common trap, and the next practice move.`;
  }
  if (lowerType.includes("video")) {
    return `An animated website lesson for ${title} with storyboard frames, transcript text, plain-English method, and mastery practice.`;
  }
  return `A complete website lesson for ${title} with original teaching copy, examples, trap warnings, and a drill handoff.`;
}

contentLibrary.push(
  ...requestedWebsiteLessonBlueprints.map(([id, title, topic, topicLabel, category, difficulty, minutes, type, skill]) => ({
    id,
    title,
    topic,
    topicLabel,
    ...
    skill,
    page: "lesson-player.html",
    description: requestedLessonDescription(title, type, skill),
  })),
);
